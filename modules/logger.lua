...
return {
  ...
  init = function(loaded, config)
    ...
    local function writer(level, module, thread, msg)
      local f = assert(fs.open(config.logger.file.value, "a"), "Cannot open log file")
      f.write(("[%5s][%s:%s] %s\n"):format(level, module, thread, msg))
      f.close()
    end
    ...
    local log = function(self, level, msg)
      ...
      writer(level, self.module, self.thread, msg)
    end
    ...
    return {
      logger = logger,
      ---@param w fun(level: string, module: string, thread: string, msg: any)
